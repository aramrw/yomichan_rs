...
use native_model::{encode, Model, native_model};
use rusqlite::params;
use tracing;
...
impl Yomichan<'_> {
    pub fn import_dictionaries<P: AsRef<Path> + Send + Sync>(
        ...
        self.db.conn.lock().execute("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)", params!["options", data_blob]).expect("Failed to update options");
...
pub fn import_dictionary<P: AsRef<Path>>(
    ...
    let tag_list: Vec<DatabaseTag> = external_data.tag_list.into_par_iter().map(|t| DatabaseTag {
        id: t.id, name: t.name, category: t.category, order: t.order, notes: t.notes, score: t.score, dictionary: t.dictionary,
    }).collect();

    let kanji_meta_list: Vec<DatabaseMetaFrequency> = external_data.kanji_meta_list.into_par_iter().map(|m| DatabaseMetaFrequency {
        id: m.id, freq_expression: m.freq_expression, mode: match m.mode {
    ...
    let kanji_list: Vec<DatabaseKanjiEntry> = external_data.kanji_list.into_par_iter().map(|k| DatabaseKanjiEntry {
        character: k.character, onyomi: k.onyomi, kunyomi: k.kunyomi, tags: k.tags, meanings: k.meanings, stats: k.stats, dictionary: k.dictionary,
    }).collect();
        ...
        let conn = conn_lock.unchecked_transaction().expect("Failed to start transaction");
        {
            ...
            for t in term_iter.by_ref().take(BATCH_SIZE) {
                let entry = DatabaseTermEntry {
                    id: t.0.clone(), expression: t.1.clone(), reading: t.2.clone(), expression_reverse: t.3.clone(), reading_reverse: t.4.clone(),
                    definition_tags: t.5.map(|s| s.to_string()), tags: t.6.map(|s| s.to_string()), rules: t.7.to_string(), score: t.8,
                    glossary: t.9.into_iter().map(|g| match g {
                        importer::structured_content::TermGlossaryGroupType::Content(c) => TermGlossaryGroupType::Content(TermGlossaryContentGroup { plain_text: c.plain_text, html: c.html }),
                        importer::structured_content::TermGlossaryGroupType::Deinflection(d) => TermGlossaryGroupType::Deinflection(TermGlossaryDeinflection { form_of: d.form_of, rules: d.rules.iter().map(|s| s.to_owned()).collect() }),
                    }).collect(),
                    sequence: t.10, term_tags: t.11.as_ref().map(|s| s.to_string()), dictionary: t.12.clone(), file_path: t.13.clone(),
                };
                let data_blob = encode(&entry).expect("Failed to encode");
                stmt.execute(params![entry.id, entry.expression, entry.reading, entry.expression_reverse, entry.reading_reverse, entry.sequence.map(|s| s as i64), entry.dictionary, data_blob]).expect("Failed to execute");
                batch_count += 1;
            }
        }
    ...
    let summary = DictionarySummary {
        title: external_data.summary.title, revision: external_data.summary.revision, sequenced: external_data.summary.sequenced, minimum_yomitan_version: external_data.summary.minimum_yomitan_version,
        version: external_data.summary.version, import_date: external_data.summary.import_date, prefix_wildcards_supported: external_data.summary.prefix_wildcards_supported,
        counts: SummaryCounts {
            terms: SummaryItemCount { total: external_data.summary.counts.terms.total },
            term_meta: SummaryMetaCount { total: external_data.summary.counts.term_meta.total, meta: MetaCounts { freq: external_data.summary.counts.term_meta.meta.freq, pitch: external_data.summary.counts.term_meta.meta.pitch, ipa: external_data.summary.counts.term_meta.meta.ipa } },
            kanji: SummaryItemCount { total: external_data.summary.counts.kanji.total },
            kanji_meta: SummaryMetaCount { total: external_data.summary.counts.kanji_meta.total, meta: MetaCounts { freq: external_data.summary.counts.kanji_meta.meta.freq, pitch: external_data.summary.counts.kanji_meta.meta.pitch, ipa: external_data.summary.counts.kanji_meta.meta.ipa } },
    ...
    let dictionary_options = DictionaryOptions {
        name: dict_name, alias: "".to_string(), enabled: true, allow_secondary_searches: true, definitions_collapsible: DictionaryDefinitionsCollapsible::default(),
        parts_of_speech_filter: false, use_deinflections: true, styles: None,
    };

    {
        let data_blob = encode(&summary).expect("Failed to encode summary");
        db.conn.lock().execute("INSERT OR REPLACE INTO summaries (title, data) VALUES (?, ?)", params![summary.title, data_blob]).expect("Failed to insert summary");
    }
            ...
            for item in term_meta_iter.by_ref().take(BATCH_SIZE) {
                let (id, term, mode, dictionary, data_blob) = match item {
                    DatabaseMetaMatchType::Frequency(freq) => {
                        ...
                        (freq.id, freq.freq_expression, "freq", freq.dictionary, data_blob)
                    },
                    DatabaseMetaMatchType::Pitch(pitch) => {
                        let data_blob = encode(&pitch).unwrap();
                        (pitch.id, pitch.pitch_expression, "pitch", pitch.dictionary, data_blob)
                    },
                    DatabaseMetaMatchType::Phonetic(ipa) => {
                        let data_blob = encode(&ipa).unwrap();
                        (ipa.id, ipa.phonetic_expression, "ipa", ipa.dictionary, data_blob)
                    },
                };
                stmt.execute(params![id, term, mode, dictionary, data_blob]).expect("Failed to execute stmt");
                batch_count += 1;
            }
        }
        if batch_count == 0 { break; }
...
fn insert_kanji_batched(db: Arc<DictionaryDatabase>, list: Vec<DatabaseKanjiEntry>, batch_size: usize) -> Result<(), rusqlite::Error> {
    ...
    loop {
        ...
        {
            let mut stmt = conn.prepare("INSERT OR REPLACE INTO kanji (character, dictionary, data) VALUES (?, ?, ?)")?;
...
fn insert_tags_batched(db: Arc<DictionaryDatabase>, list: Vec<DatabaseTag>, batch_size: usize) -> Result<(), rusqlite::Error> {
    let mut iter = list.into_iter();
    loop {
        let mut batch_count = 0;
        let mut conn_lock = db.conn.lock();
        let conn = conn_lock.unchecked_transaction()?;
        {
            let mut stmt = conn.prepare("INSERT OR REPLACE INTO tags (id, name, dictionary, data) VALUES (?, ?, ?, ?)")?;
            for item in iter.by_ref().take(batch_size) {
                let data_blob = encode(&item).unwrap();
...
fn insert_kanji_meta_batched(db: Arc<DictionaryDatabase>, list: Vec<DatabaseMetaFrequency>, batch_size: usize) -> Result<(), rusqlite::Error> {
    let mut iter = list.into_iter();
    loop {
        let mut batch_count = 0;
        let mut conn_lock = db.conn.lock();
        let conn = conn_lock.unchecked_transaction()?;
        {
            let mut stmt = conn.prepare("INSERT OR REPLACE INTO kanji_meta (character, dictionary, data) VALUES (?, ?, ?)")?;
            for item in iter.by_ref().take(batch_size) {
                let data_blob = encode(&item).unwrap();
...
fn read_dir_helper<P: AsRef<Path>>(
    ...
    for entry in fs::read_dir(path)? {
        ...
        let path = entry.path();
        if path.is_dir() {
            read_dir_helper(&path, index, tag_banks, kanji_meta_banks, kanji_banks, term_meta_banks, term_banks)?;
        } else {
            let file_name = path.file_name().and_then(|n| n.to_str()).unwrap_or("");
            if file_name.contains("term_bank") { term_banks.push(path); }
            else if file_name.contains("index.json") { *index = path; }
            else if file_name.contains("term_meta_bank") { term_meta_banks.push(path); }
            else if file_name.contains("kanji_meta_bank") { kanji_meta_banks.push(path); }
            else if file_name.contains("kanji_bank") { kanji_banks.push(path); }
            else if file_name.contains("tag_bank") { tag_banks.push(path); }
        }
    }
    Ok(())
}
