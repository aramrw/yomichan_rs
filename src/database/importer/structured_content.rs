...
pub enum TermGlossaryContent {
    ...
    StructuredContent(Box<TermGlossaryContent>),

    // deinflection is also a complex type, it's just an array in json
    // We can model it as a struct containing the vec.
    #[serde(rename = "deinflection")]
    Deinflection(TermGlossaryDeinflection),
}
