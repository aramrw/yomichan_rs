...
mod db_tests {
    ...
    use yomichan_rs::database::dictionary_database::{TermExactQueryRequest};
    ...
    fn test_search_es() {
        ...
        let ycd = Yomichan::new(db_path.parent().unwrap()).unwrap();
        ycd.set_language("en").unwrap();
        ycd.with_profile_mut(|p| p.options_mut().scanning_mut().alphanumeric = true)
            .unwrap();
